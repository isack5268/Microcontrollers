...
unsigned int timer, hourSet = 0, freshTime;
bool starting = false;
...
void loop() {
  ...
  if(timeClient.getHours() > freshTime){
  ...
  if(timeClient.getHours() < hourSet + 2 && timeClient.getHours() >= Hours(sunrise) + 1)
    starting = false;
  else
    starting = true;
...
void Parse(){
  ...
  if(httpCode == 200)
  {
   ...
   // Parse JSON object
    ...
    sunset = root["results"]["sunset"].as<String>();
